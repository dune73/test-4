...
```bash
$> wget https://archive.apache.org/dist/apr/apr-1.7.5.tar.bz2.sha256
$> sha256sum --check apr-1.7.5.tar.bz2.sha256
apr-1.7.5.tar.bz2: OK
```

The test should not result in any problems, _OK_. We can now continue with unpacking, pre-configuring and compiling _apr_.

```bash
$> tar -xvjf apr-1.7.5.tar.bz2
$> cd apr-1.7.5
...
```bash
$> cd /usr/src/apache
$> wget https://archive.apache.org/dist/httpd/httpd-2.4.62.tar.bz2
```
...
```bash
$> wget https://archive.apache.org/dist/httpd/httpd-2.4.62.tar.bz2.sha256
$> sha256sum --check httpd-2.4.62.tar.bz2.sha256
...
```bash
$> cd httpd-2.4.62
$> ./configure --prefix=/opt/apache-2.4.62  --with-apr=/usr/local/apr/bin/apr-1-config \
...
```bash
$> sudo chown -R `whoami` /opt/apache-2.4.62
```
...
```bash
$> sudo ln -s /opt/apache-2.4.62 /apache
...
Server built:   November 06 2024 13:32:29
...
Server compiled with....
 ...
 -D HTTPD_ROOT="/opt/apache-2.4.62"
 -D SUEXEC_BIN="/opt/apache-2.4.62/bin/suexec"
