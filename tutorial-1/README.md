...
```bash
$> cd /usr/src/apache
$> wget https://www-eu.apache.org/dist//httpd/httpd-2.4.43.tar.bz2
```
...
```bash
$> wget https://www.apache.org/dist/httpd/httpd-2.4.43.tar.bz2.sha256
$> sha256sum --check httpd-2.4.43.tar.bz2.sha256
...
```bash
$> sudo chown -R `whoami` /opt/apache-2.4.43
```
...
```bash
$> sudo ln -s /opt/apache-2.4.43 /apache
...
Server compiled with....
 ...
 -D HTTPD_ROOT="/opt/apache-2.4.43"
 -D SUEXEC_BIN="/opt/apache-2.4.43/bin/suexec"
